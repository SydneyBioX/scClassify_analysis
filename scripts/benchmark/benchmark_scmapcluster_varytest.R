...
run_scmapcluster <- function(train , test , train_label , test_label  , n   ){
  ...
  write.csv(True_Labels_scmapcluster,paste('test_', n ,'_scmapcluster_True.csv',sep=""), row.names = FALSE)
  write.csv(Pred_Labels_scmapcluster,paste('test_' , n , '_scmapcluster_Pred.csv',sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/scmapcluster")
  returnlist <- run_scmapcluster(test, train,  test_label, train_label, thisdata )
