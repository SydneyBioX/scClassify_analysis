...
run_scID<-function(train, test, trainLabels, testLabels,  n){
  ...
  write.csv(True_Labels_scID,paste("test_", n, "_scID_True.csv",sep=""), row.names = FALSE)
  write.csv(Pred_Labels_scID,paste("test_", n, "_scID_Pred.csv",sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  returnlist <- run_scID(test,  train, test_label, train_label, thisdata )
