...
run_singleCellNet<-function(trainData, testData, trainLabels, testLabels, n){
  ...
  write.csv(True_Labels_singleCellNet,paste( "test_", n , '_singleCellNet_True.csv', sep=""),row.names = FALSE)
  write.csv(Pred_Labels_singleCellNet,paste("test_", n ,  '_singleCellNet_Pred.csv',sep= ""),row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/singlecellnet")
  returnlist <- run_singleCellNet(test,  train, test_label,  train_label, thisdata )
