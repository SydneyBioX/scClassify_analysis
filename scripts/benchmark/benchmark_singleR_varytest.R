...
run_SingleR<-function(traindata, testdata,  trainLabels, testLabels, n){
  ...
  write.csv(True_Labels_SingleR,paste("test_", n, '_SingleR_True.csv',sep=""), row.names = FALSE)
  write.csv(Pred_Labels_SingleR,paste("test_" , n ,'_SingleR_Pred.csv',sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/singleR")
  returnlist <- run_SingleR(test,  train, test_label,  train_label, thisdata )
