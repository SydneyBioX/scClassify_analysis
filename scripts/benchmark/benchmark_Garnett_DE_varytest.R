...
run_Garnett_DE <- function(train_data, trainLabels, test_data, testLabels, n){
  ...
  write.csv(true_labels, paste("test_", n, '_Garnett_DE_True.csv' , sep=""), row.names = FALSE)
  write.csv(pred_labels, paste("test_", n, '_Garnett_DE_Pred.csv', sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/Garnett_DE")
  returnlist <- run_Garnett_DE(test, test_label, train, train_label, thisdata )
