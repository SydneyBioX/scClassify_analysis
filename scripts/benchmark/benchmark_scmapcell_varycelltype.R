...
run_scmapcell <- function(train , test , train_label , test_label , n   ){
  ...
  write.csv(True_Labels_scmapcell,paste('train_' , n , '_scmapcell_True.csv',sep=""),row.names = FALSE)
  write.csv(Pred_Labels_scmapcell,paste('train_' , n , '_scmapcell_Pred.csv',sep=""),row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/scmapcell")
  returnlist <- run_scmapcell(train, test,  train_label, test_label, thisdata )
...
setwd("/dora/nobackup/yuec/scclassify/benchmark/scmapcell/vary_celltype")
