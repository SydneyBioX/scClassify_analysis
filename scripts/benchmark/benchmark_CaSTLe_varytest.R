...
####### Does scClassify’s scale with testing size?  #####
dataset <- c(100, 200, 500, 1000, 2000, 5000,
             10000, 20000, 30000)


train <- read.csv("/albona/nobackup/biostat/datasets/singlecell/tabulaMuris_benchmark/train_2000.csv")
rownames(train) <- train[ , 1]
train <- train[ , -1]

train_label <- read.csv("/albona/nobackup/biostat/datasets/singlecell/tabulaMuris_benchmark/train_2000_label.csv")
...
run_CaSTLe<-function(train, test, trainlabels, testlabels, n){
  ...
  write.csv(True_Labels_Castle,paste("test_", n, '_True_CaSTLe.csv', sep= ""),row.names = FALSE)
  write.csv(Pred_Labels_Castle,paste("test_", n,  '_Pred_CaSTLe.csv',sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  test <- read.csv(paste0("/albona/nobackup/biostat/datasets/singlecell/tabulaMuris_benchmark/test_", thisdata, ".csv"))
  rownames(test) <- test[ , 1]
  test <- test[ , -1]
  test_label <- read.csv(paste0("/albona/nobackup/biostat/datasets/singlecell/tabulaMuris_benchmark/test_", thisdata, "_label.csv"))
