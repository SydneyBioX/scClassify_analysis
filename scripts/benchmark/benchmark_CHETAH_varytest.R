...
run_CHETAH <-function(traindata, train_label , testdata,  test_label, n ){
  ...
  write.csv(True_Labels_CHETAH,paste("test_", n, '_CHETAH_True.csv',sep=""),row.names = FALSE)
  write.csv(Pred_Labels_CHETAH,paste("test_",  n ,  '_CHETAH_Pred.csv',sep=""),row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  setwd("/dora/nobackup/yuec/scclassify/benchmark/CHETAH")
  returnlist <- run_CHETAH(test, test_label, train, train_label, thisdata )
