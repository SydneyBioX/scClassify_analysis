...
run_scPred<-function(train , test , train_label, test_label , n ){
  ...
  write.csv(True_Labels_scPred,paste("train", n, '_scPred_True.csv',sep=""), row.names = FALSE)
  write.csv(Pred_Labels_scPred,paste("train", n, '_scPred_Pred.csv',sep=""), row.names = FALSE)
...
for (i in (1:length(dataset))){
  ...
  tryCatch( {
    ...
    returnlist <- run_scPred(train,  test, train_label,  test_label, thisdata )
...
setwd("/dora/nobackup/yuec/scclassify/benchmark/scPred/vary_celltype")
